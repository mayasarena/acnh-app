...
const SeaCreatureDetails = () => {
  ...
  const fetchSeaCreatureData = () => {
    fetch(`https://api.nookipedia.com/nh/sea/${seaCreatureName}?api_key=${process.env.REACT_APP_ACNH_API_KEY}`)
    .then(response => {
  ...
  try {
    availabilityArrayN = seaCreature['north']['availability_array'];
    availabilityArrayS = seaCreature['south']['availability_array'];
  } catch {
  ...
  try {
    catchphrases = seaCreature['catchphrases'];
  } catch {
  ...
  return (
    <div className="body">
      ...
      <div className="flex gap-5 items-center justify-between m-auto p-4 md:w-5/6 lg:w-1/2 font-bold text-xl">
        ...
        <span className="grow">{seaCreature['name']}</span>
        <Starred objectName={seaCreature['name']} objectType='fish' objectImg={seaCreature['image_url']} alreadyStarred={user?.starred?.find(e => (e.objName === seaCreature['name']) && (e.objType === 'sea'))}/>
      </div>
      ...
      <div className="flex flex-col m-auto text-center bg-darkcream rounded-lg p-4 md:w-5/6 lg:w-1/2">
        <div className="bg-lightcream m-auto rounded-md p-8 mb-3">
          <img className="max-w-24 max-h-24" src={seaCreature['render_url']} />
        </div>
        <div className="flex flex-col gap-2">
          <div className="bg-lightcream rounded p-1 px-3 text-left">
            <span className="flex gap-2"><b>Category:</b> Sea Creature</span>
          </div>
          <div className="flex bg-lightcream rounded p-1 px-3 text-left">
            <div className="grow basis-1">
            <span className="flex flex-row gap-2"><b>Sell (Nook):</b> {seaCreature['sell_nook']} <img className="w-6" src={bell} alt="Bell" /></span>
            </div>
          </div>
